...
在二维XY坐标系中，向量`(x, y)`基于原点逆时针旋转 $\theta$ 角度至向量`(x', y')`，其*变换矩阵*为
...
<div align=center>
<img src="/assets/img/二维XY坐标系旋转.png" width="60%" />
</div>
...
在二维XY坐标系中，向量`(x, y)`进行缩放`n`倍得到向量`(nx, ny)`，其*变换矩阵*为
...
在二维XY坐标系中，向量的变换都可由旋转、缩放和平移进行线性组合变换，通过逆矩阵可以进行还原变换
...
<div align=center>
<img src="/assets/img/三维XYZ坐标系旋转.png" width="60%" />
</div>

1.将旋转轴OA旋转至YOZ平面内，即OA绕 y轴 旋转角度 $\alpha$ ，变换矩阵如下
...
2.将旋转轴OA旋转至Z轴上，即OA绕 x轴 旋转角度 $\beta$ ，变换矩阵如下
...
4.物体还原步骤2和步骤1的变换，即进行逆矩阵变换，所以最终变换矩阵如下
